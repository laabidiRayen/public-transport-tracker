-- Public Transport Tracker - PostgreSQL Database Initialization Script
-- Date: 2026-01-04
...
-- 1. ROUTES Table - Stores bus/train route information
CREATE TABLE routes (
...
-- 2. STATIONS Table - Stores bus stops and train stations
...
-- 3. SCHEDULES Table - Stores planned schedules for routes
...
-- 4. DELAYS Table - Stores real-time delay information
CREATE TABLE delays (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT check_delay_positive CHECK (delay_minutes >= 0)
);
...
-- 5. USERS Table - Stores user information (for future enhancements)
CREATE TABLE users (
...
-- 6. USER_FAVORITES Table - Stores user favorite routes
...
-- Insert sample schedules for BUS 101
...
-- Insert sample schedules for BUS 42
INSERT INTO schedules (route_id, departure_station_id, arrival_station_id, departure_time, arrival_time, day_of_week, frequency)
VALUES
...
-- Insert sample schedules for TRAIN A1
INSERT INTO schedules (route_id, departure_station_id, arrival_station_id, departure_time, arrival_time, day_of_week, frequency)
VALUES
...
-- Insert sample delays
INSERT INTO delays (schedule_id, delay_minutes, reason, is_active)
VALUES
    (1, 5, 'Traffic congestion on main route', TRUE),
    (2, 10, 'Mechanical issue with vehicle', FALSE),
    (4, 0, 'On schedule', TRUE),
    (8, 15, 'High passenger volume', TRUE);
...
-- Show active delays
SELECT COUNT(*) as active_delays FROM delays WHERE is_active = TRUE;
